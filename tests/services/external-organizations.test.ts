...
import {
    ...
    type ExternalOrganizationListResult,
    ...
    type ExternalOrganizationAllowedDisciplinesParams,
    type ExternalOrganizationDependentsParams,
    type ExternalOrganizationNotesParams,
    type OrderingsList,
    type Note,
    type NoteListResult,
    ...
    type UploadedFile
} from '../../src/services/external-organizations'

type PureClientLike = Pick<PureClient, 'get' | 'post' | 'put' | 'delete'>

describe('ExternalOrganizationsService', () => {
    let client: jest.Mocked<PureClientLike>
    let service: ExternalOrganizationsService

    const basePath = '/external-organizations'
    ...
    beforeEach(() => {
        ...
        service = new ExternalOrganizationsService(client)
    })
    ...
    it('lists external organizations and executes queries', async () => {
        const params = { size: 10 } as ExternalOrganizationListParams
        const config: AxiosRequestConfig = { timeout: 1500 }
        const list = { count: 5 } as unknown as ExternalOrganizationListResult
        ...
        const query = { window: { size: 2 } } as unknown as ExternalOrganizationsQuery
        const queryResult = { count: 7 } as unknown as ExternalOrganizationListResult
    ...
    it('performs CRUD operations', async () => {
        const uuid = 'external-organization-uuid'
        const payload = { uuid } as unknown as ExternalOrganization
    ...
    it('locks and unlocks an external organization', async () => {
        const uuid = 'lockable-external-organization'
    ...
    it('lists dependents', async () => {
        const uuid = 'dependents-external-organization'
        const params = { verbose: true } as ExternalOrganizationDependentsParams
    ...
    it('manages discipline associations', async () => {
        const uuid = 'discipline-external-organization'
        const scheme = 'scheme'
        ...
        const allowedParams = { size: 25 } as ExternalOrganizationAllowedDisciplinesParams
    ...
    it('handles notes and file operations', async () => {
        const uuid = 'note-external-organization'
        const params = { size: 5 } as ExternalOrganizationNotesParams
    ...
    it('supports merge and preview deduplication', async () => {
        const refs = { items: [] } as unknown as ExternalOrganizationRefList
        const merged = { uuid: 'target' } as unknown as ExternalOrganization
        const previewPayload = { items: [] } as unknown as ExternalOrganizationList
        const previewResult = { count: 1 } as unknown as ExternalOrganizationListResult

        client.post.mockResolvedValueOnce(merged)
        expect(await service.merge(refs)).toBe(merged)
        expect(client.post).toHaveBeenCalledWith(`${basePath}/merge`, refs, undefined, undefined)

        client.post.mockResolvedValueOnce(previewResult)
        expect(await service.previewDeduplication(previewPayload)).toBe(previewResult)
        expect(client.post).toHaveBeenLastCalledWith(
            `${basePath}/preview-deduplication`,
            previewPayload,
            undefined,
            undefined
        )
    ...
    it('supports custom base path', async () => {
        const customBase = '/custom-external-organizations'
        const customService = new ExternalOrganizationsService(client, { basePath: customBase })
        const list = { count: 0 } as unknown as ExternalOrganizationListResult
