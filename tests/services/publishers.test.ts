...
import {
    ...
    type PublisherDependentsParams,
    type PublisherNotesParams,
    type PublisherAllowedDisciplinesParams,
...
describe('PublishersService', () => {
    ...
    beforeEach(() => {
        ...
        service = new PublishersService(client)
    })
    ...
    it('lists publishers with params and config', async () => {
        const params = { size: 10 } as PublisherListParams
        const config: AxiosRequestConfig = { timeout: 500 }
        const result = { count: 1 } as unknown as PublisherListResult
    ...
    it('executes publishers query', async () => {
        const query = { uuids: ['publisher-1'] } as unknown as PublishersQuery
        const response = { count: 1 } as unknown as PublisherListResult
    ...
    it('retrieves, creates, updates and removes a publisher', async () => {
        const uuid = 'publisher-uuid'
        const payload = { uuid } as unknown as Publisher
    ...
    it('locks and unlocks a publisher', async () => {
        const uuid = 'locked-publisher'
    ...
    it('lists dependents with optional parameters', async () => {
        const uuid = 'publisher-with-dependents'
        const params = { verbose: true } as PublisherDependentsParams
    ...
    it('manages discipline associations', async () => {
        const uuid = 'publisher-discipline'
        ...
        const params = { size: 5 } as PublisherAllowedDisciplinesParams
    ...
    it('lists and creates notes', async () => {
        const uuid = 'publisher-notes'
        const params = { size: 3 } as PublisherNotesParams
    ...
    it('fetches allowed metadata and orderings', async () => {
        ...
        expect(client.get).toHaveBeenNthCalledWith(1, `${basePath}/allowed-countries`, undefined, undefined)

        expect(await service.getAllowedKeywordGroupConfigurations()).toBe(keywordConfigs)
        expect(client.get).toHaveBeenNthCalledWith(2, `${basePath}/allowed-keyword-group-configurations`, undefined, undefined)

        expect(await service.getAllowedKeywordGroupConfigurationClassifications(42)).toBe(classification)
        expect(client.get).toHaveBeenNthCalledWith(3, `${basePath}/allowed-keyword-group-configurations/42/classifications`, undefined, undefined)

        expect(await service.getAllowedLocales()).toBe(locales)
        expect(client.get).toHaveBeenNthCalledWith(4, `${basePath}/allowed-locales`, undefined, undefined)

        expect(await service.getAllowedTypes()).toBe(classification)
        expect(client.get).toHaveBeenNthCalledWith(5, `${basePath}/allowed-types`, undefined, undefined)

        expect(await service.getAllowedWorkflowSteps()).toBe(workflow)
        expect(client.get).toHaveBeenNthCalledWith(6, `${basePath}/allowed-workflow-steps`, undefined, undefined)

        expect(await service.getOrderings()).toBe(orderings)
        expect(client.get).toHaveBeenNthCalledWith(7, `${basePath}/orderings`, undefined, undefined)
    })

    it('supports custom base path', async () => {
        const customBase = '/custom-publishers'
        const customService = new PublishersService(client, { basePath: customBase })
        const result = { count: 0 } as unknown as PublisherListResult
